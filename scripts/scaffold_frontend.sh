...
scaffold_usage() {
    ...
    echo ""
    echo "Optional env: CREATE_VITE_VERSION (default 6.0.0), SCAFFOLD_NPM_LEGACY_PEER_DEPS=1"
}
...
# Each entry is "name@range" for npm install (scoped names use the final @ as version).
...
scaffold_main() {
    ...
    # Two npm phases:
    # (1) install only what the Vite template wrote to package.json.
    # (2) one resolver pass for all scaffold specs — dev and prod can share one npm install
    #     (--save-dev … then --save … applies flags to the following package list).
